...
using Nedev.DocxToPdf.PdfEngine.Compatibility;
    ...
    public void Convert(Stream docxStream, Stream pdfStream)
    {
        ...
        while (i < elements.Count)
        {
            ...
            foreach (var item in itemsToAdd)
            {
                ...
                // 处理浮动对象
                if (item is global::Nedev.DocxToPdf.Converters.FloatingObject floatObj)
                {
                    if (floatObj.Wrapping == global::Nedev.DocxToPdf.Converters.WrappingStyle.BehindText)
                    {
                        writer.DirectContentUnder.AddImage(floatObj.Image);
                    }
                    else if (floatObj.Wrapping == global::Nedev.DocxToPdf.Converters.WrappingStyle.InFrontOfText)
                    {
                        // 简单起见，立即绘制到前景
                        // 理想情况下应收集起来，确保在文本后绘制
                    ...
                    else if (floatObj.Wrapping == global::Nedev.DocxToPdf.Converters.WrappingStyle.TopAndBottom)
                    {
                        // 上下型：
                        // 1. 结束当前行，换行
                        // 2. 在当前 Y 位置留出图片高度
                    ...
                    else if (floatObj.Wrapping == global::Nedev.DocxToPdf.Converters.WrappingStyle.Square || 
                             floatObj.Wrapping == global::Nedev.DocxToPdf.Converters.WrappingStyle.Tight || 
                             floatObj.Wrapping == global::Nedev.DocxToPdf.Converters.WrappingStyle.Through)
                    {
                        // 四周型/紧密型/穿越型：文字环绕图片
                        // 策略：
                        // 1. 计算图片在页面中的位置
