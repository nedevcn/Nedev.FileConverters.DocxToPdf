...
public class TableConverter
{
    ...
    /// <summary>
    /// 将 DOCX Table 转为 iTextSharp PdfPTable
    /// </summary>
    ...
    private PdfPTable? BuildTable(WTable docxTable, List<TableRow> rows, float pageWidth)
    {
        ...
        // 计算列数（优先 TableGrid，其次按 GridSpan 求最大值）
        ...
        // 获取 TableLook (用于判断哪些区域需要应用标题样式，如首列、首行等)
        ...
        // 如果上述属性为 null，尝试解析 w:val (十六进制位掩码)
        ...
        bool forceBold = false)
    {
        var pdfCell = new PdfPCell
        {
            UseAscender = true,
            UseDescender = true,
            VerticalAlignment = Element.ALIGN_MIDDLE // 默认居中
        };
        ...
        // 如果是标题或加粗要求的单元格，适当增加一点内边距让文字不拥挤
        if (forceBold)
        {
    ...
    private static void SetTableBorders(PdfPTable pdfTable, TableBorders? borders)
    {
        ...
        // 注意：PdfPTable 作为一个整体，其 Border 实际上是由内部单元格构成的
        // 但我们可以设置 DefaultCell 的默认边框
    ...
    private static void SetCellBorders(PdfPCell pdfCell, TableCellProperties? cellProps, TableBorders? tableBorders, OpenXmlElement? colorScheme, int rowIndex, int colIndex, int rowCount, int columnCount, int colSpan)
    {
        ...
        // 特殊处理：如果单元格有背景色，且边框宽度为0，可能会看到微小的白缝（iTextSharp 特性）
        // 适当给一个极其微小的同色边框或者保持0
