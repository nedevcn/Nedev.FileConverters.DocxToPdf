...
public class FontHelper
{
    ...
    public string GetDefaultFontName()
    {
        ...
        // 最终 fallback：使用 Helvetica
        return "Helvetica";
    }
    /// <summary>
    /// 根据 DOCX RunProperties 获取 iTextSharp Font
    /// </summary>
    ...
    /// <summary>
    /// 根据 DOCX RunProperties 和 ParagraphMarkRunProperties 获取 iTextSharp Font
    /// </summary>
    public iTextFont GetFont(RunProperties? runProperties, ParagraphMarkRunProperties? paraRunProps, float? defaultSize = null, bool boldOverride = false)
    {
        ...
        // 缓存键 - 必须包含 boldOverride，否则强制加粗的请求可能会由于命中普通体缓存而失效
        ...
        return _fontCache.GetOrAdd(cacheKey, _ =>
        {
            ...
            // 如果是加粗，但默认字体是常规体，且系统中有黑体(SimHei)，强制用黑体作为加粗的替代
            if (wantsBold)
            {
                if (fontName != "SimHei" && fontName != "黑体")
                {
